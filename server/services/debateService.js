...
const DebateView = require("../models/DebateView");
...
// Get paginated debates created by the authenticated user
const getMyDebates = async (userId, query) => {
...
// Increment view count (permanent — one view per user per debate, enforced by unique index)
const incrementView = async (debateId, userId) => {
  try {
    // Attempt to create a new view record
    await DebateView.create({ debateId, userId });
    ...
    // Duplicate key error — user already viewed this debate
    if (err.code === 11000) {
      const debate = await Debate.findById(debateId).select("views").lean();
      if (!debate) {
        const error = new Error("Debate not found");
        error.statusCode = 404;
        throw error;
...
const deleteDebate = async (debateId, user) => {
  ...
  if (
    user.role !== "admin" &&
    debate.creator.toString() !== user._id.toString()
  ) {
