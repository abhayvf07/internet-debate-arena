...
// GET /api/debates/me
const getMyDebates = asyncHandler(async (req, res) => {
  const result = await debateService.getMyDebates(req.user._id, req.query);
  res.json(result);
});
...
const getDebateById = asyncHandler(async (req, res) => {
  const result = await debateService.getDebateById(
    req.params.id,
    req.query.userId,
  );
  res.json(result);
...
const incrementView = asyncHandler(async (req, res) => {
  const result = await debateService.incrementView(req.params.id, req.user._id);
  res.json(result);
});
